...
SCRIPT_DIR="$(dirname "$0")"
PLAYWRIGHT_SERVER_SCRIPT="$SCRIPT_DIR/../ai_recorder/node_runtime/ts_bridge/stagehand_server.js"

echo "Session started on $DISPLAY with PIDs: ${CHILD_PIDS[*]}"

# Run Playwright server in foreground - when this exits, the script exits
DISPLAY=$DISPLAY PORT=$PLAYWRIGHT_PORT node $PLAYWRIGHT_SERVER_SCRIPT
